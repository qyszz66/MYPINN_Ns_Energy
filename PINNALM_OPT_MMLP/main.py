...
def transform_loss(loss, use_sqrt, use_log):
    """根据配置对损失进行变换"""
    if use_sqrt:
        loss = torch.sqrt(loss)
    elif use_log:
        loss = torch.log(loss)  # 避免对零或负值取对数
    return loss
...
def get_opt(opt_name, opt_params, model_params):
    if opt_name == 'adam':
        ...
        return LBFGS(model_params, line_search_fn = 'strong_wolfe')
...
def paraing():
    ...
    Mu = torch.ones(1 + 1 +1 + 1, 1, device=device) # BC and PDE constraints
    mu_max = Mu * 1.
    Lambda = Mu * 1.
    Bar_v  = Lambda * 0.
...
# %%
def My_PINN(X_top_torch, X_BLR_torch, X_f_torch, XY_star_torch, T_star_torch, Ri, Re, Pr,
    para_adapt, Lambda, Mu, mu_max, Bar_v, iteration_num, use_sqrt, use_log):
    ...
    for iter in range(iteration_num):
        ...
        def _closure():
            net.eval()
            loss_cont1, loss_mom1, loss_mom2, loss_enery1 = pde(net, X_f_torch, Ri, Re, Pr)
            pde_loss = loss_cont1   
            pde1_loss =loss_enery1
            ...
            constr = torch.cat((boundary_loss, pde_loss, pde1_loss, data_loss), dim=0)
        ...
        def closure():
            ...
            loss = transform_loss(loss, use_sqrt, use_log)
        ...
        if (iter + 1) % 50 == 0:
            print(
                "n = %d, loss = %.3e, objective = %.3e, constr_loss = %.3e, %.3e, %.3e, %.3e, Lambda = %.3e,  %.3e, %.3e, %.3e, Mu = %.3e,  %.3e, %.3e, %.3e"
                % (
                    iter + 1, 
                    loss,
                    objective,
...
# %%
Pr = configuration['Pr_Ri_Re'][0]#普朗特数
Ri = configuration['Pr_Ri_Re'][1]#格拉晓夫数
Re = configuration['Pr_Ri_Re'][2]#雷诺数
...
if configuration['Flag_pretrain']:
    ...
    checkpoint = torch.load("qm458m20_Ri=6.25_Re=100.pth")
...
para_adapt = ParaAdapt(zeta=0.99, omega=0.999, eta=torch.tensor([[1.0], [0.01], [0.01], [1.0]]).to(device), epsilon=1e-16)
use_sqrt = configuration['use_sqrt']
use_log = configuration['use_log']

My_PINN(X_top_torch, X_BLR_torch, X_f_torch, XY_star_torch, T_star_torch, Ri, Re, Pr, para_adapt, Lambda, Mu, mu_max, Bar_v, iteration_num, use_sqrt, use_log)
# %%
